...
def mainfunc(c):
    lambda1 = lambda t: c * np.sqrt(t)
    t = 10000000000
    sol = solve_diffeq_1cc(2, t, lambda1)
    fht = sol.t[-1]
    return fht, sol

#shadow generating solution and driver plot with varying c
if __name__ == "__main__":
    ...
    carr = np.arange(0, 3.1, 0.1)
    T_values = []

    pool = Pool()
    T_values = pool.map(mainfunc, carr)
    pool.close()
    pool.join()
    ...
    print(len(carr))
    print(len(T_values))
    for c, sol, gold, black in zip(carr, T_values, gold15, black15):
        fht = sol[0]
        solution = sol[1]
        t_values = np.arange(0, fht, 0.000001)
        driver = lambda t: c * np.sqrt(t)
    ...
    plt.xlabel("time")
    plt.ylabel("y")
    plt.savefig("paperpics/3a c * sqrt(t) drivers + sols {c | 0 < c < 3}.png")

    plt.figure(figsize=(8, 6), dpi=200)
    plt.scatter(carr, fhts, color="black")
    plt.xlabel("c")
    plt.ylabel("FHT")
    plt.savefig("paperpics/3b c * sqrt(t) fhts {c | 0 < c < 3}.png")
